...
{
    std::ifstream fic("ficbanquise_base.txt");
    std::ifstream fic1("ficarrete1_base.txt");
    double val=0.0;
    int x=0;
    int y=0;
    std::string nom;
...
void Graph::sauver()
{
    ...
    //const car * temp ;
    //temp=nomfic_som.str() ;
    std::ifstream fic_sauv_som ("fic_banquise_sauv.txt") ;
    std::ifstream fic_sauv_arete ("fic_arete1_sauv.txt") ;
    std::ostringstream fichier ;
   ...
   // fichier_sauv <<
    ...
    std::ofstream fichier_sauv_som(fichier.str(), std::ios::out | std::ios::trunc) ; /// lecture du fichier et écrire à la fin du fichier texte en écrasant ce qui a été fait dans ce fichier
    std::ofstream fichier_sauv_arete(fichier.str(), std::ios::out | std::ios::trunc) ;
    if (fichier_sauv_arete)
    {
        std::map <int,Edge>::iterator paires ; // l'iterator va permette d'ouvrir des instances de classe paires qui se composent de la clé et de la valeur
        for (paires=m_edges.begin(); paires!=m_edges.end(); paires ++)
        {
            Edge valeur = paires -> second ;
            std::cout << valeur.m_from << " " << valeur.m_to << " " << valeur.m_weight << " " << std::endl ;
            fichier_sauv_arete <<  valeur.m_from << valeur.m_to << valeur.m_weight ;
        }
        std::cout << "Vous avez sauvegarde les sommets presents dans le graphe actuel\n" ;
    }
    else
    {
        std::cout << "EOF ! \n\n\n" ;
    ...
    if (fichier_sauv_som) /// il faut maintenant sauvegarder les artes
    {
       // std::cout <<
        ...
        {
            Vertex valeur = paires2-> second ;
            std::cout << valeur.m_value << " " << valeur.coordx << " " << valeur.coordy << " " << valeur.nom_pic  << " " << std::endl ;
            fichier_sauv_som << valeur.m_value << valeur.coordx << valeur.coordy << valeur.nom_pic ;
        }
        std::cout <<"Vous avez sauvegarde les aretes du graphe actuel \n";
    }
    else
    {
        std::cout << "EOF ! \n\n\n" ;
...
void Graph::appel_graphe_banquise()
{
