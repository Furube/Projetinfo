...
GraphInterface::GraphInterface(int x, int y, int w, int h)
{
    ...
    bouton3.set_frame(0,400,90,100);
...
///sous-programme chargement de fichier
void Graph::chargement_fic(std::string ficbanq, std::string ficarete1)
{
    ...
    int indice=0;
    int indice2=0;
///condition de vérification si le fichier existe
    if(fic)
    {
        ///récupération élément du fichier
        fic>>m_ordre;
        getline(fic,nom);
        ///redimensionne le tableau en fonction du nombre de sommet
        ...
        if(fic1)
        {
            ...
            ///boucle de récupération des valeurs dans le tableau
        ...
        ///fermeture du fichier des arrêtes
        fic1.close();

    }
    ///fermeture du fichier des sommets
    fic.close();

}

///Sous-programme d'ajout de sommet ou d'arrêtes
void Graph::ajout()
{
    ...
    ///menu console pour choisir d'ajouter une arrête ou un sommet
    do
    {
        ...
        {
        case '1':
            ///demande à l'utilisateur les données qu'ils souhaitent entrer pour un sommet
            ephemere.m_indice=0;
            do
            {
            ...
            ///ajout du nouveau sommet dans la map
        ...
        case '2':
            ///demande à l'utilisateur les données qu'ils souhaitent entrer pour une arrête
...
///sous_programme pour sauvegarder le premier graphe
...
///sous-programme sauvegarder le second graphe
...
///sous-programme sauvegarder le troisième graphe
...
///sous-programme qui lance le premier graphe
void Graph::appel_graphe_banquise_base()
{
    ...
    while ( !key[KEY_ESC] )
    {
        ...
        ///fonction ajouter lorsque le bouton est cliqué
        if(m_interface->bouton1.clicked())
        {
            this->ajout();
        }

        ///fonction sauvegarder lorsque le bouton est cliqué
        if (m_interface->bouton2.clicked())
        {
            this->sauver_banquise();
        }

        ///dynamisme de population lorsque le bouton est cliqué (bouton espace pour accélérer)
        if(key[KEY_SPACE] || m_interface->bouton3.clicked())
        {
            this->Dynamique();
        }

        ///Suppression lorsque le bouton est cliqué
...
///Sous-programme de suppression d'un sommet
void Graph::supprimer_sommet(int indice)
{
    std::map<int,Edge>::iterator it;
    std::vector<int> select;
    ///cherche parmi toutes les arrêtes celles reliés au sommet à supprimer et les stockes
    ...
    if(select.size()!=0)
    {
        ///Supprime toutes les arrêtes stockés
...
/// Sous-programme suppression du sommet
...
///sous-programme suppression des arrêtes
...
int * Graph::CFC(int ** adjacence, int som, int ordre)
{
    ...
    /// allocation des tableaux dynamiques c1 c2 et c de taille ordre
    c1 = new int [ordre] ;
    c2 = new int [ordre] ;
    c = new int [ordre] ;
...
int ** Graph::toutes_CFC(int ** adjacence, int ordre) //récupère la matrice d'adjacence
{
