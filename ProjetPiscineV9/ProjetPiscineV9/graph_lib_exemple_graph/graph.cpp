...
void Graph::update()
{
    ...
    for (auto &elt : m_vertices)
    {
        ...
        if(elt.second.get_but()==1)
        {
            indice=elt.second.get_indice();
    ...
    for(int i=0; i<m_ordre; i++)
    {
        ...
        for(int j=0; j<m_ordre; j++)
        {
            for (std::map <int, Vertex>::iterator it = m_vertices.begin(); it!= m_vertices.end(); it ++)
            {
                ...
                /* for(int k=0; k<m_edges[m_vertices].size(); k++)
                 {

                     if (m_edges[k].m_from==j)
                     {
                         adjacence[i][j]=1;
                     }
                     else
...
int ** Graph::toutes_CFC(int ** adjacence, int ordre)
{
